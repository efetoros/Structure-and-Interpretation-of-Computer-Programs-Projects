...
def free_bacon(opponent_score):
    ...
    """
    # BEGIN PROBLEM 2
    A = str(opponent_score)
    if len(A) == 2: 
        return max(int(A[0]), int(A[1])) + 1
    else:
        return int(A[0]) + 1
    # END PROBLEM 2
...
def is_prime(number):
    """ returns if True or False according to if a numb
    is prime
    >>> is_prime(13)
    True
    >>> is_prime(19)
...
def take_turn(num_rolls, opponent_score, dice=six_sided):
    ...
    # BEGIN PROBLEM 2
    if num_rolls == 0:
        bpoints = free_bacon(opponent_score)
        if is_prime(bpoints) == True:
            return next_prime(bpoints)
        else:
            return bpoints
        return free_bacon(opponent_score)
    tpoints = roll_dice(num_rolls, dice)
    if is_prime(tpoints) == True:
        if next_prime(tpoints) < 25 - num_rolls:
            return next_prime(tpoints)
        else:
            return 25 - num_rolls
    if tpoints < 25 - num_rolls:
        return tpoints
    else:
        return 25 - num_rolls
    ...
    while score0 < goal and score1 < goal:
        ...
        if player == 1:
            if strategy1(score1, score0) == -1:
                ...
                score1 += take_turn(strategy1(score1, score0), score0, select_dice(score0, score1, dice_swapped))
...
def check_strategy(strategy, goal=GOAL_SCORE):
    ...
    for i in range(0,goal):
        for x in range(0,goal):
            check_strategy_roll(i, x, strategy(i, x))
    return None
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    score = []
    for i in range (1,11):
        score.append(make_averaged(roll_dice, num_samples)(i, dice))
    for m in range(0, len(score)):
        if score[m] == max(score):
            return m + 1
    # END PROBLEM 8
...
def bacon_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    if free_bacon(opponent_score) >= margin:
        return 0
    elif is_prime(free_bacon(opponent_score)) == True and next_prime(free_bacon(opponent_score)) >= margin:
        return 0
    else:
        return num_rolls  
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    """
    # BEGIN PROBLEM 10
    if free_bacon(opponent_score) == opponent_score/2 - score:
        return 0
    else:
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    """
    num_rolls = 5
    if swap_strategy(score, opponent_score, 10, num_rolls) == 0:
        num_rolls = 0
    if opponent_score/2 - 1 == score:
        num_rolls = -1
    # if bacon_strategy(score, opponent_score, 7, num_rolls) == 0:
    #     num_rolls = 0
